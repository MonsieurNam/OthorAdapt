"""Generate the fixed Reviewer-2 three-seed SingLoRA replication matrix."""
...
PROTOCOL_SCHEMA_VERSION = "phase3.reviewer2.singlora.v1"
EXPECTED_DATASETS = [
    "fgvc",
    "eurosat",
    "food101",
...
def load_protocol(path):
    try:
        ...
        raise ValueError("Reviewer-2 protocol must be JSON-compatible YAML/JSON") from exc
...
def validate_protocol(protocol):
    ...
    for field, expected in EXPECTED_METHOD.items():
        if method.get(field) != expected:
            raise ValueError(
                f"Reviewer-2 SingLoRA {field} mismatch: expected {expected!r}, "
...
def _run_name(dataset, shot, seed):
    return f"{dataset}_{int(shot)}shot_seed{int(seed)}_test_singlora_r8_ramp100"
