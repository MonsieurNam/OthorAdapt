...
class ArcFaceLoss(nn.Module):
    ...
    def forward(self, cosine, labels):
        ...
        """
        print(f"Cosine min: {cosine.min().item():.6f}, Cosine max: {cosine.max().item():.6f}")
        ...
        output *= self.s
        if torch.isnan(F.cross_entropy(output, labels)).any():
            print("!!! WARNING: NaN detected in the loss !!!")
