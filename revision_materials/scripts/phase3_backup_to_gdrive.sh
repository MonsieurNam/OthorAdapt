#!/usr/bin/env bash
#
# Zip Phase 3 results/logs/checkpoints and upload to Google Drive every 5h.
#
# Checkpoint files are included by default, but only after they are older than
# CHECKPOINT_MIN_AGE_MINUTES to avoid files still being written by torch.save.
#
set -euo pipefail
...
REMOTE_RETENTION_DAYS="${REMOTE_RETENTION_DAYS:-7}" # delete remote backups older than N days (0 = keep all)
INCLUDE_CHECKPOINTS="${INCLUDE_CHECKPOINTS:-1}"      # 1 = include checkpoint files
CHECKPOINT_MIN_AGE_MINUTES="${CHECKPOINT_MIN_AGE_MINUTES:-10}"  # avoid files still being written
STATUS_FILE="${STATUS_FILE:-${BASE_DIR}/logs/phase3_gdrive_backup_status.env}"
LOCK_DIR="${LOCK_DIR:-${BASE_DIR}/logs/phase3_backup_to_gdrive.lock}"

# directories to back up (relative to BASE_DIR), skipped if missing.
...
acquire_lock() {
  ...
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    log "Another backup loop is already running (pid=$old_pid); exiting."
...
print_help() {
  cat <<'EOF'
Usage:
  phase3_backup_to_gdrive.sh once
  phase3_backup_to_gdrive.sh loop

Server-side Phase 3 backup loop. It creates timestamped zip snapshots under
...
Optional environment variables:
  ...
  CHECKPOINT_MIN_AGE_MINUTES=10
  STATUS_FILE=/root/OthorAdapt/revision_materials/logs/phase3_gdrive_backup_status.env
  LOCK_DIR=/root/OthorAdapt/revision_materials/logs/phase3_backup_to_gdrive.lock

Skip checkpoint files when you only need lightweight logs/results:
  INCLUDE_CHECKPOINTS=0 bash revision_materials/scripts/phase3_backup_to_gdrive.sh once
...
make_zip() {
  ...
  zip_path="${STAGING_DIR}/phase3_backup_${stamp}.zip"
  checksum_path="${zip_path}.sha256"
  filelist="${STAGING_DIR}/phase3_backup_${stamp}.files"
  mkdir -p "$STAGING_DIR"
...
prune_local() {
  ...
  local files
  mapfile -t files < <(ls -1t "${STAGING_DIR}"/phase3_backup_*.zip 2>/dev/null || true)
...
prune_remote() {
  ...
  "$RCLONE" delete "${RCLONE_REMOTE}:${GDRIVE_DIR}/" \
    --filter '+ phase3_backup_*.zip' \
    --filter '+ phase3_backup_*.zip.sha256' \
    --filter '- *' \
...
run_once() {
  ...
  if ! prune_local; then
    write_status "failure" "local pruning failed after upload" "$zip_path"
    return 1
  fi
  if ! prune_remote; then
    write_status "failure" "remote pruning failed after upload" "$zip_path"
    return 1
  fi
...
preflight() {
  if ! is_nonnegative_int "$RETENTION"; then
    log "ERROR: RETENTION must be a non-negative integer, got '$RETENTION'."
    exit 1
  fi
  if ! is_nonnegative_int "$REMOTE_RETENTION_DAYS"; then
    log "ERROR: REMOTE_RETENTION_DAYS must be a non-negative integer, got '$REMOTE_RETENTION_DAYS'."
    exit 1
  fi
  if ! is_nonnegative_int "$CHECKPOINT_MIN_AGE_MINUTES"; then
    log "ERROR: CHECKPOINT_MIN_AGE_MINUTES must be a non-negative integer, got '$CHECKPOINT_MIN_AGE_MINUTES'."
    exit 1
  fi
  if [[ "$INCLUDE_CHECKPOINTS" != "0" && "$INCLUDE_CHECKPOINTS" != "1" ]]; then
    log "ERROR: INCLUDE_CHECKPOINTS must be 0 or 1, got '$INCLUDE_CHECKPOINTS'."
    exit 1
  fi
  ...
  if ! "$RCLONE" listremotes 2>/dev/null | grep -qx "${RCLONE_REMOTE}:"; then
    ...
    log "Configure it first (see --help), then re-run."
    exit 1
  fi
}
...
case "${1:-loop}" in
  ...
  loop)
    acquire_lock
    preflight
    log "Starting backup loop (interval=${INTERVAL}). Targets: ${TARGETS[*]}; include_checkpoints=${INCLUDE_CHECKPOINTS}"
    while true; do
